...
(defvar kusto-ts-mode-font-lock-rules nil
  "Font lock rules for kusto-ts-mode.")

(defun kusto-ts-mode--setup-font-lock ()
  "Setup font lock rules for kusto-ts-mode."
  (when (and (fboundp 'treesit-font-lock-rules) (treesit-language-available-p 'kusto))
    (setq kusto-ts-mode-font-lock-rules
...
(define-derived-mode kusto-ts-mode prog-mode "Kusto"
  ...
  ;; Enable tree-sitter for this mode
  (when (and (fboundp 'treesit-parser-create) (treesit-language-available-p 'kusto))
    (treesit-parser-create 'kusto)

    ;; Setup font lock rules
    (kusto-ts-mode--setup-font-lock)
